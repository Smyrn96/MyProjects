...
void generate_relational(vmopcode op, quad* q) {
    ...
    if (q->result->numConst < q->label) {
        ...
        add_incomplete_jump(nextinstructionlabel(), q->result->numConst);
    }
...
void generate_uminus(vmopcode opcode, quad* q) {
    instruction* tempInstruction = (instruction*)malloc(sizeof(instruction));
    tempInstruction->opcode = opcode;
    tempInstruction->srcLine = q->line;
    q->taddress = nextinstructionlabel();
...
void printBinaryInstructions(FILE* f) {
    int i = 0;
    for (i = 0; i < nextinstructionlabel(); i++) {
        switch (instructions[i].opcode) {
            case assign_v:
                if (instructions[i].arg1.type == retval_a) {
                    fprintf(f, "%u \t\t%u %u\t\t %u\n", instructions[i].opcode, instructions[i].result.type, instructions[i].result.val, instructions[i].arg1.type);
                } else { /*2nd assign*/
                    fprintf(f, "%u \t\t%u %u\t\t %u %u\n", instructions[i].opcode, instructions[i].result.type, instructions[i].result.val, instructions[i].arg1.type, instructions[i].arg1.val);
                }
                break;
            case add_v:
                fprintf(f, "%u \t\t%u %u\t\t %u %u\t%u %u\n", instructions[i].opcode, instructions[i].result.type, instructions[i].result.val, instructions[i].arg1.type, instructions[i].arg1.val, instructions[i].arg2.type, instructions[i].arg2.val);
                break;
            case sub_v:
                fprintf(f, "%u \t\t%u %u\t\t %u %u\t%u %u\n", instructions[i].opcode, instructions[i].result.type, instructions[i].result.val, instructions[i].arg1.type, instructions[i].arg1.val, instructions[i].arg2.type, instructions[i].arg2.val);
                break;
            ...
            case jgt_v:
                ...
                break;
            case call_v:
                fprintf(f, "%u \t\t%u %u\t\t\n", instructions[i].opcode, instructions[i].result.type, instructions[i].result.val);
                break;
            case pusharg_v:
                fprintf(f, "%u \t\t%u %u\t\t\n", instructions[i].opcode, instructions[i].result.type, instructions[i].result.val);
                break;
...
unsigned totalglobals(FILE* f) {
    fprintf(f, "\n\n#Globals: %u\n\n", numOfGlobals);
}
unsigned magicnumber(FILE* f) {
    unsigned MAGICNUMBER = 340200501;
    fprintf(f, "MAGICNUMBER: %u\n\n", MAGICNUMBER);
    return 1;
}
void string(unsigned i, FILE* f) {
    fprintf(f, "%u\t\t%s\n", strlen(stringConsts[i]), stringConsts[i]);
}
unsigned strings(FILE* f) {
    unsigned i;
    fprintf(f, "#Strings: %u\n", currStringConst);
    fprintf(f, "size\tdata\n");
...
void number(unsigned i, FILE* f) {
    fprintf(f, "%.3f\n", numConsts[i]);
}

unsigned numbers(FILE* f) {
    unsigned i;
    fprintf(f, "#Numbers: %u\n", currNumConst);
    fprintf(f, "data\n");
...
void userfuncs(unsigned i, FILE* f) {
    fprintf(f, "%u\t\t%u\t\t\t%s\n", userFuncs[i].address, userFuncs[i].localSize, userFuncs[i].id);
}

unsigned userfunctions(FILE* f) {
    unsigned i;
    fprintf(f, "#UserFunctions: %u\n", currUserfunct);
    fprintf(f, "address\tlocalsize\tid\n");
    for (i = 0; i < currUserfunct; i++) {
        userfuncs(i, f);
    }
    fprintf(f, "\n");
    return 1;
...
void libfuncs(unsigned i, FILE* f) {
    fprintf(f, "%u\t\t%s\n", strlen(namedLibfuncs[i]), namedLibfuncs[i]);
}

unsigned libfunctions(FILE* f) {
    unsigned i;
    fprintf(f, "#LibraryFunctions: %u\n", currLibfunct);
    fprintf(f, "size\tid\n");
    for (i = 0; i < currLibfunct; i++) {
        libfuncs(i, f);
    }
    fprintf(f, "\n");
    return 1;
...
unsigned arrays(FILE* f) {
    return strings(f) && numbers(f) && userfunctions(f) && libfunctions(f);
}

unsigned instr_code(FILE* f) {
    fprintf(f, "#Instructions: %u\n\n", nextinstructionlabel());
    fprintf(f, "opcode\tresult\t arg1\targ2\n");
    printBinaryInstructions(f);
    return 1;
}

unsigned avmbinaryfile(FILE* f) {
    return magicnumber(f) && arrays(f) && instr_code(f) && totalglobals(f);
}