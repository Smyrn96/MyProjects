...
unsigned first_cmd_stmt = 1; /* flag gia tin prwti entoli enos statement gia na ginoun 0 ta lines twn quad ektos tou 1ou.*/
...
typedef enum iopcode {
    ...
    not,
    or
    , and, param, call, getretval, funcstart, ret, funcend
} iopcode;
...
void emit(iopcode op, expr* result, expr* arg1, expr* arg2, unsigned label, unsigned line) {
    ...
    if (arg1 != NULL && arg1->sym != NULL && istempname(arg1->sym->name)) {
        arg1->charConst = arg1->sym->name;
    }
    p->arg1 = arg1;
    if (arg2 != NULL && arg2->sym != NULL && istempname(arg2->sym->name)) {
        arg2->charConst = arg2->sym->name;
    }
...
symrec_t* newtemp() {
    ...
    if (sym == NULL) {
        if (scope != 0) {
            sym = insert(scope, 0, LOCALVAR, name);
        } else {
            sym = insert(scope, 0, GLOBAL, name);
        }
        extraSets(sym);
        return sym;
    } else {
...
void print_relops(char* op, int i, FILE* f) {
    if ((quads + i)->arg1->type == constbool_e || (quads + i)->arg1->type == conststring_e || (quads + i)->arg2->type == constbool_e || (quads + i)->arg2->type == conststring_e) {
