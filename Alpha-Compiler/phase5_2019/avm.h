...
-ARITHMETICS
-ASSIGNS
-CALLS (xwris libfuncs kai anadromi)
-PUSHARG
-FUNCENTER
-FUNCEXIT
-INPUT
-Tables (getelem setelem) FULL MODARISMENA
*/
#include <math.h>
...
void read_userFuncs(FILE* f) {
    ...
    unsigned tmp_address;
    unsigned tmp_localsize;
    ...
    while (i < userfunctotals) {
        fscanf(f, "%u%u%s", &tmp_address, &tmp_localsize, buf);
...
void libfunc_input() {} /* <- haha auti edw den 8a ginei pote */
...
void execute_tablegetelem(instruction* instr) {
    ...
    printf("type %d val %d\n", t->type, t->data.numVal);
    printf("type instr %d val instr %d\n", instr->arg1.type, instr->arg1.val);
